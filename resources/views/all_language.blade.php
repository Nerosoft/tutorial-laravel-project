                            ...
                            @if(is_array($items))
                                @foreach($items as $key3=>$items)
                                    <tr>
                                        ...
                                        <th>{{$items}}</th>
                                        @include('all_model.table_array', ['idModel'=>'editModel'.$index, 'title'=>$lang->title3, 'idForm'=>'editForm'.$index, 'button'=>$lang->button3, 'action'=>route('edit.editAllLanguage', ['lang'=>$myNameLang, 'id'=>$key, 'name'=>$key2, 'item'=>$key3])])
                                    </tr>
                                @endforeach
                            @elseif($key === 'Html')
                                <tr>
                    ...
                    @if(is_array($items))
                        @foreach($items as $key3=>$items)
                            <tr>
                                <th>{{$index++}}</th>
                                <th>{{$items}}</th>
                                @include('all_model.table_array', ['idModel'=>'editModel'.$index, 'title'=>$lang->title3, 'idForm'=>'editForm'.$index, 'button'=>$lang->button3, 'action'=>route('edit.editAllLanguage', ['lang'=>request()->route('lang'), 'id'=>request()->route('id'), 'name'=>$key, 'item'=>$key3])])
                            </tr>
                        @endforeach
